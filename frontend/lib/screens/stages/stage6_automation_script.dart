...
class _Stage6AutomationScriptState extends State<Stage6AutomationScript> {
        ...
        reader.onLoadEnd.listen((e) async {
          ...
          if (content != null && content.isNotEmpty) {
            ...
            await controller.updateWorkflowStage(2, 'pending', {
              ...
              'test_automation_content_approved': false,
            });
  ...
  Widget build(BuildContext context) {
    ...
    return Obx(() {
      ...
      final stageContent = wf?.stageData['test_automation_content'] as String?;
      final brdContent = wf?.stageData['brd_content'] as String?;
      final isApproved = wf?.stageData['test_automation_content_approved'] == true ||
      ...
      children: [
        ...
        Expanded(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text('Test Automation Script', style: GoogleFonts.outfit(fontSize: 24, fontWeight: FontWeight.bold, color: EnterpriseTheme.getTextPrimary(isDark))),
              const SizedBox(height: 3),
              Text('Automated test scripts using modern frameworks',
          ...
          child: Row(
            ...
            children: [
              ...
              Text(
                isApproved
                    ? 'Approved'
                    : (hasContent ? 'Automation Script Ready (Pending Review)' : 'Pending Generation'),
          ...
          children: [
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text('Test Cases', style: GoogleFonts.inter(fontSize: 12, fontWeight: FontWeight.w600, color: EnterpriseTheme.getTextPrimary(isDark))),
                        Text(
            ...
            SizedBox(
              ...
              child: _buildGradientButton(
                ...
                label: isGenerating ? 'Generating...' : (stageContent == null ? 'Generate Automation Script' : 'Regenerate Test Automation Script'),
                isDark: isDark,
            ...
            if (_isContextSourcesExpanded) ...[
              ...
              const SizedBox(height: 8),
              _buildContextChip(Icons.smart_toy_rounded, 'Architecture', 'Event-Driven Microservices', true, isDark),
              const SizedBox(height: 8),
      ...
      child: Column(
        ...
        children: [
          ...
          Container(
            ...
            child: Row(
              children: [
                Icon(Icons.smart_toy_rounded, size: 16, color: EnterpriseTheme.indigo),
                const SizedBox(width: 8),
                Text('Test Automation Script Output', style: GoogleFonts.outfit(fontSize: 14, fontWeight: FontWeight.w700, color: EnterpriseTheme.getTextPrimary(isDark))),
                if (isApproved) ...[
                  const SizedBox(width: 8),
                  Container(
                      ...
                      children: [
                        ...
                        Expanded(
                          child: TextField(
                            ...
                            decoration: InputDecoration(
                              hintText: 'Enter or paste Test Automation Script markdown content here...',
      ...
      child: Column(
        ...
        children: [
          ...
          SizedBox(
            width: 380,
            child: Text(
              'Click "Generate Automation Script" to synthesize a comprehensive Test Automation Script, or upload an existing design document to review, edit, and approve.',
      ...
      child: Row(
        children: [
          ...
          Text(
            stageContent != null
                ? (isApproved
                    ? 'Test Automation Script approved and locked • ${stageContent.split('\n').length} lines'
                    : 'Test Automation Script generated • ${stageContent.split('\n').length} lines (Pending Approval)')
            ...
            if (isApproved) ...[
              ...
              if (_isEditing) ...[
                _buildGradientButton(
                  onPressed: () async {
                    ...
                    await controller.updateWorkflowStage(2, 'approved', {
                      'test_automation_content': content,
                      'test_automation_content_approved': true,
                    });
                    setState(() {
                      _isEditing = false;
                    });
                ...
                _buildGradientButton(
                  onPressed: () async {
                    await controller.updateWorkflowStage(2, 'approved', {
                      'test_automation_content': stageContent,
                      'test_automation_content_approved': true,
                    });
                    controller.logTerminal('Test Automation Script approved and confirmed.', level: 'SUCCESS');
                    if (mounted) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Row(
  ...
  Future<void> _generate(EnterpriseSDLCController controller, feature) async {
    ...
    try {
      final payload = {
        ...
        'targetStage': 6,
        'testAutomationPrompt': _promptCtrl.text,
        'memoryMd': feature.memoryMd,
      };
      final res = await ApiService.generateDeliverables(payload);
      
      String ddContent = "Failed to generate test automation script.";
      if (res['deliverables'] != null && res['deliverables'].length > 6) {
        ddContent = res['deliverables'][6]['markdownContent'];
